...
elseif($currentAmbient['ambientType'] == 'SALA_TEL' || $currentAmbient['ambientType']=='PLANCIA' || $currentAmbient['ambientType'] == 'INFERMERIA')
{
	...
	$resPgPresenti = mysql_query("SELECT pgID, pgUser, pgAvatar FROM pg_users WHERE pgRoom = '".$currentAmbient['locID']."' AND pgLastAct >= ".($curTime-1800)." ORDER BY pgUser");
...
$adminCondition = (PG::mapPermissions("SM",$currentUser->pgAuthOMA)) ? '' : "AND (type <> 'MASTERSPEC' OR sender = ".$_SESSION['pgID']." OR specReceiver = ".$_SESSION['pgID'].")";

$chatLines = mysql_query("SELECT IDE,chat,time,type FROM federation_chat WHERE ambient = '$ambient' AND time > $maxTime AND (type <> 'SPECIFIC' OR sender = ".$_SESSION['pgID'].") $adminCondition ORDER BY time ASC");
