...
function setterCA(data)
{
	...
	if(data['TEMP']) {jQuery('#reminderTemp').text(data['TEMP']+'°C');}
}
...
function rollDice(){
	...
	jQuery.post(
		'ajax_manipulateAbilities.php?action=roll',
		{
			amb: jQuery('#ambientID').prop('value'),
			luckypoint: htluckypoint,
			abID: jQuery('#selectedDiceSkill').val()
		},
		function(e){
