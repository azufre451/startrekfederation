...
$res= mysql_query("SELECT pgID FROM `pg_users` WHERE pgLastAct < $oneMonth AND pgBavo=0 AND pgAuthOMA <> 'BAN' AND png = 0");
$SITAbavo = "";
$tabavosize = 0; 
...
if ($tabavosize)
	$SITA .= "> Bavosizzo <span style=\"font-weight:bold;color:#096bd0\">$tabavosize</span> PG inattivi da più di UN mese:<p style=\"margin:15px;\">$SITAbavo<br />Ora sono tutti dei vermi verminosi.<br />";


$res= mysql_query("SELECT pgID FROM `pg_users` WHERE pgLastAct < $twoMonth AND pgAuthOMA <> 'BAN' AND pgBavo=1  AND png = 0");
$SITAcanc = "";
$tadelete = 0; 
